 ...
 * switched on. The Cloudflare tag added by hand fared no better: it reports to
 * /cdn-cgi/rum on this origin, the Worker answers 404, the fallback is refused
 * by CORS, and it recorded nothing at all. What counts visits now is the
 * pro-motion beacon, so that is what this looks for — and for this project's
 * id on it, since a tag copied from another site counts somebody else's.
 */
const PM_BEACON = /<script[^>]*src="[^"]*\/pm\.js"[^>]*data-project="dovego-it"/;
...
export const analyticsCheck = async (fetchFn: FetchFn, origin: string): Promise<CheckResult> => {
  ...
  if (!PM_BEACON.test(body)) return bad(id, title, 'no pro-motion beacon on the page — nothing is being counted');
  return ok(id, title, 'the pro-motion beacon is on the page');
};
